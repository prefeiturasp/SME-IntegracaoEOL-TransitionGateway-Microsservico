...
from apps.institucional.libs.gateway_client import get_client
...
class InstitucionalProxyView(APIView):
    def get(self, request: Request, path: str = "") -> Response:
        ...
        try:
            ...
            logger.error("[institucional] Circuit breaker aberto")
            return Response(
                {"erro": "Serviço institucional temporariamente indisponível"},
                status=503,
            )
        except (RetryError, Exception) as exc:
            logger.error("[institucional] Falha na comunicação: %s", exc)
            return Response(
