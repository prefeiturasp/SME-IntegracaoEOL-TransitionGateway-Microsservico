...
from apps.professores.libs.gateway_client import get_client
...
class ProfessoresProxyView(APIView):
    def get(self, request: Request, path: str = "") -> Response:
        ...
        try:
            ...
            logger.error("[professores] Circuit breaker aberto")
            return Response(
                {"erro": "Serviço professores temporariamente indisponível"},
                status=503,
            )
        except (RetryError, Exception) as exc:
            logger.error("[professores] Falha na comunicação: %s", exc)
            return Response(
