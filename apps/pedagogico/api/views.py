...
from apps.pedagogico.libs.gateway_client import get_client

logger = logging.getLogger("gateway_apps")


class HealthPedagogicoView(APIView):
...
class PedagogicoProxyView(APIView):
    def get(self, request: Request, path: str = "") -> Response:
        ...
        try:
            ...
            logger.error("[pedagogico] Circuit breaker aberto")
            return Response(
                {"erro": "Serviço pedagógico temporariamente indisponível"},
                status=503,
            )
        except (RetryError, Exception) as exc:
            logger.error("[pedagogico] Falha na comunicação: %s", exc)
            return Response(
